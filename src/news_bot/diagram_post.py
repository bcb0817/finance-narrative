...
投稿本文のURLよりも、読者が理解できる解説画像を優先します。
図解価値を1〜10点で評価してください。7点以上にできるのは、次のいずれかの構造を
...
因果・比較・時系列は数値がなくても、根拠のある説明材料が3点以上あれば画像化の対象です。
数値が少ないことだけを理由に減点しないでください。
以下は必ず6点以下です。
- 単一の出来事や発言を言い換えるだけ
- 具体的な数値・比較対象・因果関係が不足している
- 複数指標の図解なのに数値が1個以下、または情報不足を推測で補う必要がある
